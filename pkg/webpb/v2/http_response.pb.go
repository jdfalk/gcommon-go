// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: webpb/v2/http_response.proto

package v2

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	v2 "github.com/jdfalk/gcommon/pkg/commonpb/v2"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	anypb "google.golang.org/protobuf/types/known/anypb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// HttpResponse represents an HTTP response from web services.
// Captures all essential HTTP response information including status,
// headers, body, and metadata for service mesh and proxy use cases.
//
// Follows 1-1-1 pattern: one message per file.
type HttpResponse struct {
	state                                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_StatusCode                int32                  `protobuf:"varint,1,opt,name=status_code,json=statusCode"`
	xxx_hidden_StatusMessage             *string                `protobuf:"bytes,2,opt,name=status_message,json=statusMessage"`
	xxx_hidden_ProtocolVersion           *string                `protobuf:"bytes,3,opt,name=protocol_version,json=protocolVersion"`
	xxx_hidden_RequestMetadata           *v2.RequestMetadata    `protobuf:"bytes,11,opt,name=request_metadata,json=requestMetadata"`
	xxx_hidden_Headers                   map[string]string      `protobuf:"bytes,12,rep,name=headers" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_Body                      *anypb.Any             `protobuf:"bytes,13,opt,name=body"`
	xxx_hidden_Cookies                   map[string]string      `protobuf:"bytes,14,rep,name=cookies" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_ContentLength             int64                  `protobuf:"varint,15,opt,name=content_length,json=contentLength"`
	xxx_hidden_ContentType               *string                `protobuf:"bytes,16,opt,name=content_type,json=contentType"`
	xxx_hidden_ContentEncoding           *string                `protobuf:"bytes,17,opt,name=content_encoding,json=contentEncoding"`
	xxx_hidden_CacheControl              *string                `protobuf:"bytes,18,opt,name=cache_control,json=cacheControl"`
	xxx_hidden_Etag                      *string                `protobuf:"bytes,19,opt,name=etag"`
	xxx_hidden_Location                  *string                `protobuf:"bytes,20,opt,name=location"`
	xxx_hidden_Server                    *string                `protobuf:"bytes,21,opt,name=server"`
	xxx_hidden_AccessControlAllowOrigin  *string                `protobuf:"bytes,22,opt,name=access_control_allow_origin,json=accessControlAllowOrigin"`
	xxx_hidden_AccessControlAllowMethods *string                `protobuf:"bytes,23,opt,name=access_control_allow_methods,json=accessControlAllowMethods"`
	xxx_hidden_AccessControlAllowHeaders *string                `protobuf:"bytes,24,opt,name=access_control_allow_headers,json=accessControlAllowHeaders"`
	xxx_hidden_RequestId                 *string                `protobuf:"bytes,25,opt,name=request_id,json=requestId"`
	xxx_hidden_ServiceName               *string                `protobuf:"bytes,26,opt,name=service_name,json=serviceName"`
	xxx_hidden_ServiceVersion            *string                `protobuf:"bytes,27,opt,name=service_version,json=serviceVersion"`
	xxx_hidden_ProcessingTimeMs          int64                  `protobuf:"varint,28,opt,name=processing_time_ms,json=processingTimeMs"`
	xxx_hidden_ServedFromCache           bool                   `protobuf:"varint,29,opt,name=served_from_cache,json=servedFromCache"`
	xxx_hidden_Streaming                 bool                   `protobuf:"varint,30,opt,name=streaming"`
	xxx_hidden_KeepAlive                 bool                   `protobuf:"varint,31,opt,name=keep_alive,json=keepAlive"`
	xxx_hidden_Compressed                bool                   `protobuf:"varint,32,opt,name=compressed"`
	xxx_hidden_CompressionRatio          float32                `protobuf:"fixed32,33,opt,name=compression_ratio,json=compressionRatio"`
	xxx_hidden_Error                     *v2.Error              `protobuf:"bytes,61,opt,name=error"`
	xxx_hidden_ProcessingStartedAt       *timestamppb.Timestamp `protobuf:"bytes,51,opt,name=processing_started_at,json=processingStartedAt"`
	xxx_hidden_GeneratedAt               *timestamppb.Timestamp `protobuf:"bytes,52,opt,name=generated_at,json=generatedAt"`
	xxx_hidden_SentAt                    *timestamppb.Timestamp `protobuf:"bytes,53,opt,name=sent_at,json=sentAt"`
	XXX_raceDetectHookData               protoimpl.RaceDetectHookData
	XXX_presence                         [1]uint32
	unknownFields                        protoimpl.UnknownFields
	sizeCache                            protoimpl.SizeCache
}

func (x *HttpResponse) Reset() {
	*x = HttpResponse{}
	mi := &file_webpb_v2_http_response_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HttpResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HttpResponse) ProtoMessage() {}

func (x *HttpResponse) ProtoReflect() protoreflect.Message {
	mi := &file_webpb_v2_http_response_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *HttpResponse) GetStatusCode() int32 {
	if x != nil {
		return x.xxx_hidden_StatusCode
	}
	return 0
}

func (x *HttpResponse) GetStatusMessage() string {
	if x != nil {
		if x.xxx_hidden_StatusMessage != nil {
			return *x.xxx_hidden_StatusMessage
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetProtocolVersion() string {
	if x != nil {
		if x.xxx_hidden_ProtocolVersion != nil {
			return *x.xxx_hidden_ProtocolVersion
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetRequestMetadata() *v2.RequestMetadata {
	if x != nil {
		return x.xxx_hidden_RequestMetadata
	}
	return nil
}

func (x *HttpResponse) GetHeaders() map[string]string {
	if x != nil {
		return x.xxx_hidden_Headers
	}
	return nil
}

func (x *HttpResponse) GetBody() *anypb.Any {
	if x != nil {
		return x.xxx_hidden_Body
	}
	return nil
}

func (x *HttpResponse) GetCookies() map[string]string {
	if x != nil {
		return x.xxx_hidden_Cookies
	}
	return nil
}

func (x *HttpResponse) GetContentLength() int64 {
	if x != nil {
		return x.xxx_hidden_ContentLength
	}
	return 0
}

func (x *HttpResponse) GetContentType() string {
	if x != nil {
		if x.xxx_hidden_ContentType != nil {
			return *x.xxx_hidden_ContentType
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetContentEncoding() string {
	if x != nil {
		if x.xxx_hidden_ContentEncoding != nil {
			return *x.xxx_hidden_ContentEncoding
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetCacheControl() string {
	if x != nil {
		if x.xxx_hidden_CacheControl != nil {
			return *x.xxx_hidden_CacheControl
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetEtag() string {
	if x != nil {
		if x.xxx_hidden_Etag != nil {
			return *x.xxx_hidden_Etag
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetLocation() string {
	if x != nil {
		if x.xxx_hidden_Location != nil {
			return *x.xxx_hidden_Location
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetServer() string {
	if x != nil {
		if x.xxx_hidden_Server != nil {
			return *x.xxx_hidden_Server
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetAccessControlAllowOrigin() string {
	if x != nil {
		if x.xxx_hidden_AccessControlAllowOrigin != nil {
			return *x.xxx_hidden_AccessControlAllowOrigin
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetAccessControlAllowMethods() string {
	if x != nil {
		if x.xxx_hidden_AccessControlAllowMethods != nil {
			return *x.xxx_hidden_AccessControlAllowMethods
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetAccessControlAllowHeaders() string {
	if x != nil {
		if x.xxx_hidden_AccessControlAllowHeaders != nil {
			return *x.xxx_hidden_AccessControlAllowHeaders
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetRequestId() string {
	if x != nil {
		if x.xxx_hidden_RequestId != nil {
			return *x.xxx_hidden_RequestId
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetServiceName() string {
	if x != nil {
		if x.xxx_hidden_ServiceName != nil {
			return *x.xxx_hidden_ServiceName
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetServiceVersion() string {
	if x != nil {
		if x.xxx_hidden_ServiceVersion != nil {
			return *x.xxx_hidden_ServiceVersion
		}
		return ""
	}
	return ""
}

func (x *HttpResponse) GetProcessingTimeMs() int64 {
	if x != nil {
		return x.xxx_hidden_ProcessingTimeMs
	}
	return 0
}

func (x *HttpResponse) GetServedFromCache() bool {
	if x != nil {
		return x.xxx_hidden_ServedFromCache
	}
	return false
}

func (x *HttpResponse) GetStreaming() bool {
	if x != nil {
		return x.xxx_hidden_Streaming
	}
	return false
}

func (x *HttpResponse) GetKeepAlive() bool {
	if x != nil {
		return x.xxx_hidden_KeepAlive
	}
	return false
}

func (x *HttpResponse) GetCompressed() bool {
	if x != nil {
		return x.xxx_hidden_Compressed
	}
	return false
}

func (x *HttpResponse) GetCompressionRatio() float32 {
	if x != nil {
		return x.xxx_hidden_CompressionRatio
	}
	return 0
}

func (x *HttpResponse) GetError() *v2.Error {
	if x != nil {
		return x.xxx_hidden_Error
	}
	return nil
}

func (x *HttpResponse) GetProcessingStartedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_ProcessingStartedAt
	}
	return nil
}

func (x *HttpResponse) GetGeneratedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_GeneratedAt
	}
	return nil
}

func (x *HttpResponse) GetSentAt() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_SentAt
	}
	return nil
}

func (x *HttpResponse) SetStatusCode(v int32) {
	x.xxx_hidden_StatusCode = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 30)
}

func (x *HttpResponse) SetStatusMessage(v string) {
	x.xxx_hidden_StatusMessage = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 30)
}

func (x *HttpResponse) SetProtocolVersion(v string) {
	x.xxx_hidden_ProtocolVersion = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 30)
}

func (x *HttpResponse) SetRequestMetadata(v *v2.RequestMetadata) {
	x.xxx_hidden_RequestMetadata = v
}

func (x *HttpResponse) SetHeaders(v map[string]string) {
	x.xxx_hidden_Headers = v
}

func (x *HttpResponse) SetBody(v *anypb.Any) {
	x.xxx_hidden_Body = v
}

func (x *HttpResponse) SetCookies(v map[string]string) {
	x.xxx_hidden_Cookies = v
}

func (x *HttpResponse) SetContentLength(v int64) {
	x.xxx_hidden_ContentLength = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 30)
}

func (x *HttpResponse) SetContentType(v string) {
	x.xxx_hidden_ContentType = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 30)
}

func (x *HttpResponse) SetContentEncoding(v string) {
	x.xxx_hidden_ContentEncoding = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 9, 30)
}

func (x *HttpResponse) SetCacheControl(v string) {
	x.xxx_hidden_CacheControl = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 10, 30)
}

func (x *HttpResponse) SetEtag(v string) {
	x.xxx_hidden_Etag = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 11, 30)
}

func (x *HttpResponse) SetLocation(v string) {
	x.xxx_hidden_Location = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 12, 30)
}

func (x *HttpResponse) SetServer(v string) {
	x.xxx_hidden_Server = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 13, 30)
}

func (x *HttpResponse) SetAccessControlAllowOrigin(v string) {
	x.xxx_hidden_AccessControlAllowOrigin = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 14, 30)
}

func (x *HttpResponse) SetAccessControlAllowMethods(v string) {
	x.xxx_hidden_AccessControlAllowMethods = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 15, 30)
}

func (x *HttpResponse) SetAccessControlAllowHeaders(v string) {
	x.xxx_hidden_AccessControlAllowHeaders = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 16, 30)
}

func (x *HttpResponse) SetRequestId(v string) {
	x.xxx_hidden_RequestId = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 17, 30)
}

func (x *HttpResponse) SetServiceName(v string) {
	x.xxx_hidden_ServiceName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 18, 30)
}

func (x *HttpResponse) SetServiceVersion(v string) {
	x.xxx_hidden_ServiceVersion = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 19, 30)
}

func (x *HttpResponse) SetProcessingTimeMs(v int64) {
	x.xxx_hidden_ProcessingTimeMs = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 20, 30)
}

func (x *HttpResponse) SetServedFromCache(v bool) {
	x.xxx_hidden_ServedFromCache = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 21, 30)
}

func (x *HttpResponse) SetStreaming(v bool) {
	x.xxx_hidden_Streaming = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 22, 30)
}

func (x *HttpResponse) SetKeepAlive(v bool) {
	x.xxx_hidden_KeepAlive = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 23, 30)
}

func (x *HttpResponse) SetCompressed(v bool) {
	x.xxx_hidden_Compressed = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 24, 30)
}

func (x *HttpResponse) SetCompressionRatio(v float32) {
	x.xxx_hidden_CompressionRatio = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 25, 30)
}

func (x *HttpResponse) SetError(v *v2.Error) {
	x.xxx_hidden_Error = v
}

func (x *HttpResponse) SetProcessingStartedAt(v *timestamppb.Timestamp) {
	x.xxx_hidden_ProcessingStartedAt = v
}

func (x *HttpResponse) SetGeneratedAt(v *timestamppb.Timestamp) {
	x.xxx_hidden_GeneratedAt = v
}

func (x *HttpResponse) SetSentAt(v *timestamppb.Timestamp) {
	x.xxx_hidden_SentAt = v
}

func (x *HttpResponse) HasStatusCode() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *HttpResponse) HasStatusMessage() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *HttpResponse) HasProtocolVersion() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *HttpResponse) HasRequestMetadata() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_RequestMetadata != nil
}

func (x *HttpResponse) HasBody() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Body != nil
}

func (x *HttpResponse) HasContentLength() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *HttpResponse) HasContentType() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *HttpResponse) HasContentEncoding() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 9)
}

func (x *HttpResponse) HasCacheControl() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 10)
}

func (x *HttpResponse) HasEtag() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 11)
}

func (x *HttpResponse) HasLocation() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 12)
}

func (x *HttpResponse) HasServer() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 13)
}

func (x *HttpResponse) HasAccessControlAllowOrigin() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 14)
}

func (x *HttpResponse) HasAccessControlAllowMethods() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 15)
}

func (x *HttpResponse) HasAccessControlAllowHeaders() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 16)
}

func (x *HttpResponse) HasRequestId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 17)
}

func (x *HttpResponse) HasServiceName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 18)
}

func (x *HttpResponse) HasServiceVersion() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 19)
}

func (x *HttpResponse) HasProcessingTimeMs() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 20)
}

func (x *HttpResponse) HasServedFromCache() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 21)
}

func (x *HttpResponse) HasStreaming() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 22)
}

func (x *HttpResponse) HasKeepAlive() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 23)
}

func (x *HttpResponse) HasCompressed() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 24)
}

func (x *HttpResponse) HasCompressionRatio() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 25)
}

func (x *HttpResponse) HasError() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Error != nil
}

func (x *HttpResponse) HasProcessingStartedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ProcessingStartedAt != nil
}

func (x *HttpResponse) HasGeneratedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_GeneratedAt != nil
}

func (x *HttpResponse) HasSentAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_SentAt != nil
}

func (x *HttpResponse) ClearStatusCode() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_StatusCode = 0
}

func (x *HttpResponse) ClearStatusMessage() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_StatusMessage = nil
}

func (x *HttpResponse) ClearProtocolVersion() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_ProtocolVersion = nil
}

func (x *HttpResponse) ClearRequestMetadata() {
	x.xxx_hidden_RequestMetadata = nil
}

func (x *HttpResponse) ClearBody() {
	x.xxx_hidden_Body = nil
}

func (x *HttpResponse) ClearContentLength() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_ContentLength = 0
}

func (x *HttpResponse) ClearContentType() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_ContentType = nil
}

func (x *HttpResponse) ClearContentEncoding() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 9)
	x.xxx_hidden_ContentEncoding = nil
}

func (x *HttpResponse) ClearCacheControl() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 10)
	x.xxx_hidden_CacheControl = nil
}

func (x *HttpResponse) ClearEtag() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 11)
	x.xxx_hidden_Etag = nil
}

func (x *HttpResponse) ClearLocation() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 12)
	x.xxx_hidden_Location = nil
}

func (x *HttpResponse) ClearServer() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 13)
	x.xxx_hidden_Server = nil
}

func (x *HttpResponse) ClearAccessControlAllowOrigin() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 14)
	x.xxx_hidden_AccessControlAllowOrigin = nil
}

func (x *HttpResponse) ClearAccessControlAllowMethods() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 15)
	x.xxx_hidden_AccessControlAllowMethods = nil
}

func (x *HttpResponse) ClearAccessControlAllowHeaders() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 16)
	x.xxx_hidden_AccessControlAllowHeaders = nil
}

func (x *HttpResponse) ClearRequestId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 17)
	x.xxx_hidden_RequestId = nil
}

func (x *HttpResponse) ClearServiceName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 18)
	x.xxx_hidden_ServiceName = nil
}

func (x *HttpResponse) ClearServiceVersion() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 19)
	x.xxx_hidden_ServiceVersion = nil
}

func (x *HttpResponse) ClearProcessingTimeMs() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 20)
	x.xxx_hidden_ProcessingTimeMs = 0
}

func (x *HttpResponse) ClearServedFromCache() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 21)
	x.xxx_hidden_ServedFromCache = false
}

func (x *HttpResponse) ClearStreaming() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 22)
	x.xxx_hidden_Streaming = false
}

func (x *HttpResponse) ClearKeepAlive() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 23)
	x.xxx_hidden_KeepAlive = false
}

func (x *HttpResponse) ClearCompressed() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 24)
	x.xxx_hidden_Compressed = false
}

func (x *HttpResponse) ClearCompressionRatio() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 25)
	x.xxx_hidden_CompressionRatio = 0
}

func (x *HttpResponse) ClearError() {
	x.xxx_hidden_Error = nil
}

func (x *HttpResponse) ClearProcessingStartedAt() {
	x.xxx_hidden_ProcessingStartedAt = nil
}

func (x *HttpResponse) ClearGeneratedAt() {
	x.xxx_hidden_GeneratedAt = nil
}

func (x *HttpResponse) ClearSentAt() {
	x.xxx_hidden_SentAt = nil
}

type HttpResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// *
	// HTTP status code (200, 404, 500, etc.).
	StatusCode *int32
	// *
	// HTTP status message/reason phrase (OK, Not Found, Internal Server Error, etc.).
	StatusMessage *string
	// *
	// HTTP protocol version used for the response.
	ProtocolVersion *string
	// *
	// Request processing metadata including timing, request ID,
	// and other observability information.
	RequestMetadata *v2.RequestMetadata
	// *
	// HTTP response headers as key-value pairs.
	// Includes standard headers like Content-Type, Cache-Control, etc.
	Headers map[string]string
	// *
	// Response body/payload data.
	// Can contain any content type (JSON, XML, HTML, binary, etc.).
	Body *anypb.Any
	// *
	// Cookies to be set in the client.
	Cookies map[string]string
	// *
	// Content length in bytes.
	ContentLength *int64
	// *
	// Content type of the response body.
	ContentType *string
	// *
	// Content encoding (gzip, deflate, etc.).
	ContentEncoding *string
	// *
	// Cache-Control header value.
	CacheControl *string
	// *
	// ETag header for caching and conditional requests.
	Etag *string
	// *
	// Location header for redirects.
	Location *string
	// *
	// Server header identifying the server software.
	Server *string
	// *
	// CORS Access-Control-Allow-Origin header.
	AccessControlAllowOrigin *string
	// *
	// CORS Access-Control-Allow-Methods header.
	AccessControlAllowMethods *string
	// *
	// CORS Access-Control-Allow-Headers header.
	AccessControlAllowHeaders *string
	// *
	// Request ID that was processed (for correlation).
	RequestId *string
	// *
	// Service name that generated this response.
	ServiceName *string
	// *
	// Service version that generated this response.
	ServiceVersion *string
	// *
	// Response processing time in milliseconds.
	ProcessingTimeMs *int64
	// *
	// Whether the response was served from cache.
	ServedFromCache *bool
	// *
	// Whether the response is being streamed.
	Streaming *bool
	// *
	// Whether the connection will be kept alive.
	KeepAlive *bool
	// *
	// Whether the response was compressed.
	Compressed *bool
	// *
	// Compression ratio if compressed (0.0-1.0).
	CompressionRatio *float32
	// *
	// Error information if the response represents an error
	// or if there were issues generating the response.
	Error *v2.Error
	// *
	// When response generation started.
	ProcessingStartedAt *timestamppb.Timestamp
	// *
	// When the response was generated.
	GeneratedAt *timestamppb.Timestamp
	// *
	// When the response was sent to the client.
	SentAt *timestamppb.Timestamp
}

func (b0 HttpResponse_builder) Build() *HttpResponse {
	m0 := &HttpResponse{}
	b, x := &b0, m0
	_, _ = b, x
	if b.StatusCode != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 30)
		x.xxx_hidden_StatusCode = *b.StatusCode
	}
	if b.StatusMessage != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 30)
		x.xxx_hidden_StatusMessage = b.StatusMessage
	}
	if b.ProtocolVersion != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 30)
		x.xxx_hidden_ProtocolVersion = b.ProtocolVersion
	}
	x.xxx_hidden_RequestMetadata = b.RequestMetadata
	x.xxx_hidden_Headers = b.Headers
	x.xxx_hidden_Body = b.Body
	x.xxx_hidden_Cookies = b.Cookies
	if b.ContentLength != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 30)
		x.xxx_hidden_ContentLength = *b.ContentLength
	}
	if b.ContentType != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 30)
		x.xxx_hidden_ContentType = b.ContentType
	}
	if b.ContentEncoding != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 9, 30)
		x.xxx_hidden_ContentEncoding = b.ContentEncoding
	}
	if b.CacheControl != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 10, 30)
		x.xxx_hidden_CacheControl = b.CacheControl
	}
	if b.Etag != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 11, 30)
		x.xxx_hidden_Etag = b.Etag
	}
	if b.Location != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 12, 30)
		x.xxx_hidden_Location = b.Location
	}
	if b.Server != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 13, 30)
		x.xxx_hidden_Server = b.Server
	}
	if b.AccessControlAllowOrigin != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 14, 30)
		x.xxx_hidden_AccessControlAllowOrigin = b.AccessControlAllowOrigin
	}
	if b.AccessControlAllowMethods != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 15, 30)
		x.xxx_hidden_AccessControlAllowMethods = b.AccessControlAllowMethods
	}
	if b.AccessControlAllowHeaders != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 16, 30)
		x.xxx_hidden_AccessControlAllowHeaders = b.AccessControlAllowHeaders
	}
	if b.RequestId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 17, 30)
		x.xxx_hidden_RequestId = b.RequestId
	}
	if b.ServiceName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 18, 30)
		x.xxx_hidden_ServiceName = b.ServiceName
	}
	if b.ServiceVersion != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 19, 30)
		x.xxx_hidden_ServiceVersion = b.ServiceVersion
	}
	if b.ProcessingTimeMs != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 20, 30)
		x.xxx_hidden_ProcessingTimeMs = *b.ProcessingTimeMs
	}
	if b.ServedFromCache != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 21, 30)
		x.xxx_hidden_ServedFromCache = *b.ServedFromCache
	}
	if b.Streaming != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 22, 30)
		x.xxx_hidden_Streaming = *b.Streaming
	}
	if b.KeepAlive != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 23, 30)
		x.xxx_hidden_KeepAlive = *b.KeepAlive
	}
	if b.Compressed != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 24, 30)
		x.xxx_hidden_Compressed = *b.Compressed
	}
	if b.CompressionRatio != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 25, 30)
		x.xxx_hidden_CompressionRatio = *b.CompressionRatio
	}
	x.xxx_hidden_Error = b.Error
	x.xxx_hidden_ProcessingStartedAt = b.ProcessingStartedAt
	x.xxx_hidden_GeneratedAt = b.GeneratedAt
	x.xxx_hidden_SentAt = b.SentAt
	return m0
}

var File_webpb_v2_http_response_proto protoreflect.FileDescriptor

const file_webpb_v2_http_response_proto_rawDesc = "" +
	"\n" +
	"\x1cwebpb/v2/http_response.proto\x12\x06web.v2\x1a\x17commonpb/v2/error.proto\x1a\"commonpb/v2/request_metadata.proto\x1a\x19google/protobuf/any.proto\x1a!google/protobuf/go_features.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1bbuf/validate/validate.proto\"\xcd\v\n" +
	"\fHttpResponse\x12\x1f\n" +
	"\vstatus_code\x18\x01 \x01(\x05R\n" +
	"statusCode\x12%\n" +
	"\x0estatus_message\x18\x02 \x01(\tR\rstatusMessage\x12)\n" +
	"\x10protocol_version\x18\x03 \x01(\tR\x0fprotocolVersion\x12E\n" +
	"\x10request_metadata\x18\v \x01(\v2\x1a.common.v2.RequestMetadataR\x0frequestMetadata\x12;\n" +
	"\aheaders\x18\f \x03(\v2!.web.v2.HttpResponse.HeadersEntryR\aheaders\x12(\n" +
	"\x04body\x18\r \x01(\v2\x14.google.protobuf.AnyR\x04body\x12;\n" +
	"\acookies\x18\x0e \x03(\v2!.web.v2.HttpResponse.CookiesEntryR\acookies\x12%\n" +
	"\x0econtent_length\x18\x0f \x01(\x03R\rcontentLength\x12!\n" +
	"\fcontent_type\x18\x10 \x01(\tR\vcontentType\x12)\n" +
	"\x10content_encoding\x18\x11 \x01(\tR\x0fcontentEncoding\x12#\n" +
	"\rcache_control\x18\x12 \x01(\tR\fcacheControl\x12\x12\n" +
	"\x04etag\x18\x13 \x01(\tR\x04etag\x12\x1a\n" +
	"\blocation\x18\x14 \x01(\tR\blocation\x12\x16\n" +
	"\x06server\x18\x15 \x01(\tR\x06server\x12=\n" +
	"\x1baccess_control_allow_origin\x18\x16 \x01(\tR\x18accessControlAllowOrigin\x12?\n" +
	"\x1caccess_control_allow_methods\x18\x17 \x01(\tR\x19accessControlAllowMethods\x12?\n" +
	"\x1caccess_control_allow_headers\x18\x18 \x01(\tR\x19accessControlAllowHeaders\x12\x1d\n" +
	"\n" +
	"request_id\x18\x19 \x01(\tR\trequestId\x12,\n" +
	"\fservice_name\x18\x1a \x01(\tB\t\xbaH\x06r\x04\x10\x01\x18dR\vserviceName\x12'\n" +
	"\x0fservice_version\x18\x1b \x01(\tR\x0eserviceVersion\x12,\n" +
	"\x12processing_time_ms\x18\x1c \x01(\x03R\x10processingTimeMs\x12*\n" +
	"\x11served_from_cache\x18\x1d \x01(\bR\x0fservedFromCache\x12\x1c\n" +
	"\tstreaming\x18\x1e \x01(\bR\tstreaming\x12\x1d\n" +
	"\n" +
	"keep_alive\x18\x1f \x01(\bR\tkeepAlive\x12\x1e\n" +
	"\n" +
	"compressed\x18  \x01(\bR\n" +
	"compressed\x12+\n" +
	"\x11compression_ratio\x18! \x01(\x02R\x10compressionRatio\x12&\n" +
	"\x05error\x18= \x01(\v2\x10.common.v2.ErrorR\x05error\x12N\n" +
	"\x15processing_started_at\x183 \x01(\v2\x1a.google.protobuf.TimestampR\x13processingStartedAt\x12=\n" +
	"\fgenerated_at\x184 \x01(\v2\x1a.google.protobuf.TimestampR\vgeneratedAt\x123\n" +
	"\asent_at\x185 \x01(\v2\x1a.google.protobuf.TimestampR\x06sentAt\x1a:\n" +
	"\fHeadersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a:\n" +
	"\fCookiesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\x88\x01\n" +
	"\n" +
	"com.web.v2B\x11HttpResponseProtoP\x01Z&github.com/jdfalk/gcommon/pkg/webpb/v2\xa2\x02\x03WXX\xaa\x02\x06Web.V2\xca\x02\x06Web\\V2\xe2\x02\x12Web\\V2\\GPBMetadata\xea\x02\aWeb::V2\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_webpb_v2_http_response_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
var file_webpb_v2_http_response_proto_goTypes = []any{
	(*HttpResponse)(nil),          // 0: web.v2.HttpResponse
	nil,                           // 1: web.v2.HttpResponse.HeadersEntry
	nil,                           // 2: web.v2.HttpResponse.CookiesEntry
	(*v2.RequestMetadata)(nil),    // 3: common.v2.RequestMetadata
	(*anypb.Any)(nil),             // 4: google.protobuf.Any
	(*v2.Error)(nil),              // 5: common.v2.Error
	(*timestamppb.Timestamp)(nil), // 6: google.protobuf.Timestamp
}
var file_webpb_v2_http_response_proto_depIdxs = []int32{
	3, // 0: web.v2.HttpResponse.request_metadata:type_name -> common.v2.RequestMetadata
	1, // 1: web.v2.HttpResponse.headers:type_name -> web.v2.HttpResponse.HeadersEntry
	4, // 2: web.v2.HttpResponse.body:type_name -> google.protobuf.Any
	2, // 3: web.v2.HttpResponse.cookies:type_name -> web.v2.HttpResponse.CookiesEntry
	5, // 4: web.v2.HttpResponse.error:type_name -> common.v2.Error
	6, // 5: web.v2.HttpResponse.processing_started_at:type_name -> google.protobuf.Timestamp
	6, // 6: web.v2.HttpResponse.generated_at:type_name -> google.protobuf.Timestamp
	6, // 7: web.v2.HttpResponse.sent_at:type_name -> google.protobuf.Timestamp
	8, // [8:8] is the sub-list for method output_type
	8, // [8:8] is the sub-list for method input_type
	8, // [8:8] is the sub-list for extension type_name
	8, // [8:8] is the sub-list for extension extendee
	0, // [0:8] is the sub-list for field type_name
}

func init() { file_webpb_v2_http_response_proto_init() }
func file_webpb_v2_http_response_proto_init() {
	if File_webpb_v2_http_response_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_webpb_v2_http_response_proto_rawDesc), len(file_webpb_v2_http_response_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_webpb_v2_http_response_proto_goTypes,
		DependencyIndexes: file_webpb_v2_http_response_proto_depIdxs,
		MessageInfos:      file_webpb_v2_http_response_proto_msgTypes,
	}.Build()
	File_webpb_v2_http_response_proto = out.File
	file_webpb_v2_http_response_proto_goTypes = nil
	file_webpb_v2_http_response_proto_depIdxs = nil
}
