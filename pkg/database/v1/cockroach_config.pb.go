// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: database/v1/cockroach_config.proto

package database

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// CockroachConfig provides CockroachDB-specific connection configuration.
// Includes retry behavior and identification options for robust connections.
type CockroachConfig struct {
	state                         protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Host               *string                `protobuf:"bytes,1,opt,name=host"`
	xxx_hidden_Port               int32                  `protobuf:"varint,2,opt,name=port"`
	xxx_hidden_User               *string                `protobuf:"bytes,3,opt,name=user"`
	xxx_hidden_Password           *string                `protobuf:"bytes,4,opt,name=password"`
	xxx_hidden_Database           *string                `protobuf:"bytes,5,opt,name=database"`
	xxx_hidden_SslMode            *string                `protobuf:"bytes,6,opt,name=ssl_mode,json=sslMode"`
	xxx_hidden_ApplicationName    *string                `protobuf:"bytes,7,opt,name=application_name,json=applicationName"`
	xxx_hidden_RetryBackoffFactor float32                `protobuf:"fixed32,8,opt,name=retry_backoff_factor,json=retryBackoffFactor"`
	xxx_hidden_MaxRetries         int32                  `protobuf:"varint,9,opt,name=max_retries,json=maxRetries"`
	XXX_raceDetectHookData        protoimpl.RaceDetectHookData
	XXX_presence                  [1]uint32
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *CockroachConfig) Reset() {
	*x = CockroachConfig{}
	mi := &file_database_v1_cockroach_config_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CockroachConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CockroachConfig) ProtoMessage() {}

func (x *CockroachConfig) ProtoReflect() protoreflect.Message {
	mi := &file_database_v1_cockroach_config_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *CockroachConfig) GetHost() string {
	if x != nil {
		if x.xxx_hidden_Host != nil {
			return *x.xxx_hidden_Host
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetPort() int32 {
	if x != nil {
		return x.xxx_hidden_Port
	}
	return 0
}

func (x *CockroachConfig) GetUser() string {
	if x != nil {
		if x.xxx_hidden_User != nil {
			return *x.xxx_hidden_User
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetPassword() string {
	if x != nil {
		if x.xxx_hidden_Password != nil {
			return *x.xxx_hidden_Password
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetDatabase() string {
	if x != nil {
		if x.xxx_hidden_Database != nil {
			return *x.xxx_hidden_Database
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetSslMode() string {
	if x != nil {
		if x.xxx_hidden_SslMode != nil {
			return *x.xxx_hidden_SslMode
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetApplicationName() string {
	if x != nil {
		if x.xxx_hidden_ApplicationName != nil {
			return *x.xxx_hidden_ApplicationName
		}
		return ""
	}
	return ""
}

func (x *CockroachConfig) GetRetryBackoffFactor() float32 {
	if x != nil {
		return x.xxx_hidden_RetryBackoffFactor
	}
	return 0
}

func (x *CockroachConfig) GetMaxRetries() int32 {
	if x != nil {
		return x.xxx_hidden_MaxRetries
	}
	return 0
}

func (x *CockroachConfig) SetHost(v string) {
	x.xxx_hidden_Host = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 9)
}

func (x *CockroachConfig) SetPort(v int32) {
	x.xxx_hidden_Port = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 9)
}

func (x *CockroachConfig) SetUser(v string) {
	x.xxx_hidden_User = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 9)
}

func (x *CockroachConfig) SetPassword(v string) {
	x.xxx_hidden_Password = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 9)
}

func (x *CockroachConfig) SetDatabase(v string) {
	x.xxx_hidden_Database = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 9)
}

func (x *CockroachConfig) SetSslMode(v string) {
	x.xxx_hidden_SslMode = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 9)
}

func (x *CockroachConfig) SetApplicationName(v string) {
	x.xxx_hidden_ApplicationName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 9)
}

func (x *CockroachConfig) SetRetryBackoffFactor(v float32) {
	x.xxx_hidden_RetryBackoffFactor = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 9)
}

func (x *CockroachConfig) SetMaxRetries(v int32) {
	x.xxx_hidden_MaxRetries = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 9)
}

func (x *CockroachConfig) HasHost() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *CockroachConfig) HasPort() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *CockroachConfig) HasUser() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *CockroachConfig) HasPassword() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *CockroachConfig) HasDatabase() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *CockroachConfig) HasSslMode() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *CockroachConfig) HasApplicationName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *CockroachConfig) HasRetryBackoffFactor() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *CockroachConfig) HasMaxRetries() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *CockroachConfig) ClearHost() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Host = nil
}

func (x *CockroachConfig) ClearPort() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_Port = 0
}

func (x *CockroachConfig) ClearUser() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_User = nil
}

func (x *CockroachConfig) ClearPassword() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_Password = nil
}

func (x *CockroachConfig) ClearDatabase() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_Database = nil
}

func (x *CockroachConfig) ClearSslMode() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_SslMode = nil
}

func (x *CockroachConfig) ClearApplicationName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_ApplicationName = nil
}

func (x *CockroachConfig) ClearRetryBackoffFactor() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_RetryBackoffFactor = 0
}

func (x *CockroachConfig) ClearMaxRetries() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_MaxRetries = 0
}

type CockroachConfig_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Host is the database host.
	Host *string
	// Port is the database port.
	Port *int32
	// User is the database user.
	User *string
	// Password is the database password.
	Password *string
	// Database is the database name.
	Database *string
	// SSLMode is the SSL mode.
	SslMode *string
	// ApplicationName is the application name.
	ApplicationName *string
	// RetryBackoffFactor is the retry backoff factor.
	RetryBackoffFactor *float32
	// MaxRetries is the maximum number of retries.
	MaxRetries *int32
}

func (b0 CockroachConfig_builder) Build() *CockroachConfig {
	m0 := &CockroachConfig{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Host != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 9)
		x.xxx_hidden_Host = b.Host
	}
	if b.Port != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 9)
		x.xxx_hidden_Port = *b.Port
	}
	if b.User != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 9)
		x.xxx_hidden_User = b.User
	}
	if b.Password != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 9)
		x.xxx_hidden_Password = b.Password
	}
	if b.Database != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 9)
		x.xxx_hidden_Database = b.Database
	}
	if b.SslMode != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 9)
		x.xxx_hidden_SslMode = b.SslMode
	}
	if b.ApplicationName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 9)
		x.xxx_hidden_ApplicationName = b.ApplicationName
	}
	if b.RetryBackoffFactor != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 9)
		x.xxx_hidden_RetryBackoffFactor = *b.RetryBackoffFactor
	}
	if b.MaxRetries != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 9)
		x.xxx_hidden_MaxRetries = *b.MaxRetries
	}
	return m0
}

var File_database_v1_cockroach_config_proto protoreflect.FileDescriptor

const file_database_v1_cockroach_config_proto_rawDesc = "" +
	"\n" +
	"\"database/v1/cockroach_config.proto\x12\vdatabase.v1\x1a!google/protobuf/go_features.proto\x1a\x1bbuf/validate/validate.proto\"\xa9\x02\n" +
	"\x0fCockroachConfig\x12\x12\n" +
	"\x04host\x18\x01 \x01(\tR\x04host\x12\x12\n" +
	"\x04port\x18\x02 \x01(\x05R\x04port\x12\x12\n" +
	"\x04user\x18\x03 \x01(\tR\x04user\x12\x1a\n" +
	"\bpassword\x18\x04 \x01(\tR\bpassword\x12\x1a\n" +
	"\bdatabase\x18\x05 \x01(\tR\bdatabase\x12\x19\n" +
	"\bssl_mode\x18\x06 \x01(\tR\asslMode\x124\n" +
	"\x10application_name\x18\a \x01(\tB\t\xbaH\x06r\x04\x10\x01\x18dR\x0fapplicationName\x120\n" +
	"\x14retry_backoff_factor\x18\b \x01(\x02R\x12retryBackoffFactor\x12\x1f\n" +
	"\vmax_retries\x18\t \x01(\x05R\n" +
	"maxRetriesB0Z&github.com/jdfalk/gcommon/pkg/database\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_database_v1_cockroach_config_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_database_v1_cockroach_config_proto_goTypes = []any{
	(*CockroachConfig)(nil), // 0: database.v1.CockroachConfig
}
var file_database_v1_cockroach_config_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_database_v1_cockroach_config_proto_init() }
func file_database_v1_cockroach_config_proto_init() {
	if File_database_v1_cockroach_config_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_database_v1_cockroach_config_proto_rawDesc), len(file_database_v1_cockroach_config_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_database_v1_cockroach_config_proto_goTypes,
		DependencyIndexes: file_database_v1_cockroach_config_proto_depIdxs,
		MessageInfos:      file_database_v1_cockroach_config_proto_msgTypes,
	}.Build()
	File_database_v1_cockroach_config_proto = out.File
	file_database_v1_cockroach_config_proto_goTypes = nil
	file_database_v1_cockroach_config_proto_depIdxs = nil
}
