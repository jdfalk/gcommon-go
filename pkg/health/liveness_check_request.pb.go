// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: health/v1/liveness_check_request.proto

package health

import (
	common "github.com/jdfalk/gcommon/pkg/common"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// LivenessCheckRequest requests a liveness check for a service.
// Liveness checks verify that a service is alive and responsive.
// Follows 1-1-1 pattern: one message per file.
type LivenessCheckRequest struct {
	state                    protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_ServiceName   *string                 `protobuf:"bytes,1,opt,name=service_name,json=serviceName"`
	xxx_hidden_Metadata      *common.RequestMetadata `protobuf:"bytes,2,opt,name=metadata"`
	xxx_hidden_Timeout       *durationpb.Duration    `protobuf:"bytes,3,opt,name=timeout"`
	xxx_hidden_ProbeEndpoint *string                 `protobuf:"bytes,4,opt,name=probe_endpoint,json=probeEndpoint"`
	XXX_raceDetectHookData   protoimpl.RaceDetectHookData
	XXX_presence             [1]uint32
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *LivenessCheckRequest) Reset() {
	*x = LivenessCheckRequest{}
	mi := &file_health_v1_liveness_check_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LivenessCheckRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LivenessCheckRequest) ProtoMessage() {}

func (x *LivenessCheckRequest) ProtoReflect() protoreflect.Message {
	mi := &file_health_v1_liveness_check_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *LivenessCheckRequest) GetServiceName() string {
	if x != nil {
		if x.xxx_hidden_ServiceName != nil {
			return *x.xxx_hidden_ServiceName
		}
		return ""
	}
	return ""
}

func (x *LivenessCheckRequest) GetMetadata() *common.RequestMetadata {
	if x != nil {
		return x.xxx_hidden_Metadata
	}
	return nil
}

func (x *LivenessCheckRequest) GetTimeout() *durationpb.Duration {
	if x != nil {
		return x.xxx_hidden_Timeout
	}
	return nil
}

func (x *LivenessCheckRequest) GetProbeEndpoint() string {
	if x != nil {
		if x.xxx_hidden_ProbeEndpoint != nil {
			return *x.xxx_hidden_ProbeEndpoint
		}
		return ""
	}
	return ""
}

func (x *LivenessCheckRequest) SetServiceName(v string) {
	x.xxx_hidden_ServiceName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 4)
}

func (x *LivenessCheckRequest) SetMetadata(v *common.RequestMetadata) {
	x.xxx_hidden_Metadata = v
}

func (x *LivenessCheckRequest) SetTimeout(v *durationpb.Duration) {
	x.xxx_hidden_Timeout = v
}

func (x *LivenessCheckRequest) SetProbeEndpoint(v string) {
	x.xxx_hidden_ProbeEndpoint = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 4)
}

func (x *LivenessCheckRequest) HasServiceName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *LivenessCheckRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Metadata != nil
}

func (x *LivenessCheckRequest) HasTimeout() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Timeout != nil
}

func (x *LivenessCheckRequest) HasProbeEndpoint() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *LivenessCheckRequest) ClearServiceName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_ServiceName = nil
}

func (x *LivenessCheckRequest) ClearMetadata() {
	x.xxx_hidden_Metadata = nil
}

func (x *LivenessCheckRequest) ClearTimeout() {
	x.xxx_hidden_Timeout = nil
}

func (x *LivenessCheckRequest) ClearProbeEndpoint() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_ProbeEndpoint = nil
}

type LivenessCheckRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Service name to check liveness for
	ServiceName *string
	// Request metadata for authentication and tracing
	Metadata *common.RequestMetadata
	// Maximum time to wait for the liveness check
	Timeout *durationpb.Duration
	// Specific probe endpoint to check (optional)
	ProbeEndpoint *string
}

func (b0 LivenessCheckRequest_builder) Build() *LivenessCheckRequest {
	m0 := &LivenessCheckRequest{}
	b, x := &b0, m0
	_, _ = b, x
	if b.ServiceName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 4)
		x.xxx_hidden_ServiceName = b.ServiceName
	}
	x.xxx_hidden_Metadata = b.Metadata
	x.xxx_hidden_Timeout = b.Timeout
	if b.ProbeEndpoint != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 4)
		x.xxx_hidden_ProbeEndpoint = b.ProbeEndpoint
	}
	return m0
}

var File_health_v1_liveness_check_request_proto protoreflect.FileDescriptor

const file_health_v1_liveness_check_request_proto_rawDesc = "" +
	"\n" +
	"&health/v1/liveness_check_request.proto\x12\thealth.v1\x1a common/v1/request_metadata.proto\x1a\x1egoogle/protobuf/duration.proto\x1a!google/protobuf/go_features.proto\"\xcd\x01\n" +
	"\x14LivenessCheckRequest\x12!\n" +
	"\fservice_name\x18\x01 \x01(\tR\vserviceName\x126\n" +
	"\bmetadata\x18\x02 \x01(\v2\x1a.common.v1.RequestMetadataR\bmetadata\x123\n" +
	"\atimeout\x18\x03 \x01(\v2\x19.google.protobuf.DurationR\atimeout\x12%\n" +
	"\x0eprobe_endpoint\x18\x04 \x01(\tR\rprobeEndpointB.Z$github.com/jdfalk/gcommon/pkg/health\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_health_v1_liveness_check_request_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_health_v1_liveness_check_request_proto_goTypes = []any{
	(*LivenessCheckRequest)(nil),   // 0: health.v1.LivenessCheckRequest
	(*common.RequestMetadata)(nil), // 1: common.v1.RequestMetadata
	(*durationpb.Duration)(nil),    // 2: google.protobuf.Duration
}
var file_health_v1_liveness_check_request_proto_depIdxs = []int32{
	1, // 0: health.v1.LivenessCheckRequest.metadata:type_name -> common.v1.RequestMetadata
	2, // 1: health.v1.LivenessCheckRequest.timeout:type_name -> google.protobuf.Duration
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_health_v1_liveness_check_request_proto_init() }
func file_health_v1_liveness_check_request_proto_init() {
	if File_health_v1_liveness_check_request_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_health_v1_liveness_check_request_proto_rawDesc), len(file_health_v1_liveness_check_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_health_v1_liveness_check_request_proto_goTypes,
		DependencyIndexes: file_health_v1_liveness_check_request_proto_depIdxs,
		MessageInfos:      file_health_v1_liveness_check_request_proto_msgTypes,
	}.Build()
	File_health_v1_liveness_check_request_proto = out.File
	file_health_v1_liveness_check_request_proto_goTypes = nil
	file_health_v1_liveness_check_request_proto_depIdxs = nil
}
