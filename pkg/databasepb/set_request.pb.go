// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: databasepb/v1/set_request.proto

package databasepb

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	commonpb "github.com/jdfalk/gcommon/pkg/commonpb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	anypb "google.golang.org/protobuf/types/known/anypb"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// Request to store a value in the cache.
// Supports flexible expiration policies and namespace isolation.
type SetRequest struct {
	state                    protoimpl.MessageState    `protogen:"opaque.v1"`
	xxx_hidden_Key           *string                   `protobuf:"bytes,1,opt,name=key"`
	xxx_hidden_Value         *anypb.Any                `protobuf:"bytes,2,opt,name=value"`
	xxx_hidden_Namespace     *string                   `protobuf:"bytes,3,opt,name=namespace"`
	xxx_hidden_Ttl           *durationpb.Duration      `protobuf:"bytes,4,opt,name=ttl"`
	xxx_hidden_Metadata      *commonpb.RequestMetadata `protobuf:"bytes,5,opt,name=metadata"`
	xxx_hidden_Overwrite     bool                      `protobuf:"varint,6,opt,name=overwrite"`
	xxx_hidden_EntryMetadata map[string]string         `protobuf:"bytes,7,rep,name=entry_metadata,json=entryMetadata" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Deprecated: Do not use. This will be deleted in the near future.
	XXX_lazyUnmarshalInfo  protoimpl.LazyUnmarshalInfo
	XXX_raceDetectHookData protoimpl.RaceDetectHookData
	XXX_presence           [1]uint32
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *SetRequest) Reset() {
	*x = SetRequest{}
	mi := &file_databasepb_v1_set_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetRequest) ProtoMessage() {}

func (x *SetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_databasepb_v1_set_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *SetRequest) GetKey() string {
	if x != nil {
		if x.xxx_hidden_Key != nil {
			return *x.xxx_hidden_Key
		}
		return ""
	}
	return ""
}

func (x *SetRequest) GetValue() *anypb.Any {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 1) {
			if protoimpl.X.AtomicCheckPointerIsNil(&x.xxx_hidden_Value) {
				protoimpl.X.UnmarshalField(x, 2)
			}
			var rv *anypb.Any
			protoimpl.X.AtomicLoadPointer(protoimpl.Pointer(&x.xxx_hidden_Value), protoimpl.Pointer(&rv))
			return rv
		}
	}
	return nil
}

func (x *SetRequest) GetNamespace() string {
	if x != nil {
		if x.xxx_hidden_Namespace != nil {
			return *x.xxx_hidden_Namespace
		}
		return ""
	}
	return ""
}

func (x *SetRequest) GetTtl() *durationpb.Duration {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 3) {
			if protoimpl.X.AtomicCheckPointerIsNil(&x.xxx_hidden_Ttl) {
				protoimpl.X.UnmarshalField(x, 4)
			}
			var rv *durationpb.Duration
			protoimpl.X.AtomicLoadPointer(protoimpl.Pointer(&x.xxx_hidden_Ttl), protoimpl.Pointer(&rv))
			return rv
		}
	}
	return nil
}

func (x *SetRequest) GetMetadata() *commonpb.RequestMetadata {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 4) {
			if protoimpl.X.AtomicCheckPointerIsNil(&x.xxx_hidden_Metadata) {
				protoimpl.X.UnmarshalField(x, 5)
			}
			var rv *commonpb.RequestMetadata
			protoimpl.X.AtomicLoadPointer(protoimpl.Pointer(&x.xxx_hidden_Metadata), protoimpl.Pointer(&rv))
			return rv
		}
	}
	return nil
}

func (x *SetRequest) GetOverwrite() bool {
	if x != nil {
		return x.xxx_hidden_Overwrite
	}
	return false
}

func (x *SetRequest) GetEntryMetadata() map[string]string {
	if x != nil {
		return x.xxx_hidden_EntryMetadata
	}
	return nil
}

func (x *SetRequest) SetKey(v string) {
	x.xxx_hidden_Key = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 7)
}

func (x *SetRequest) SetValue(v *anypb.Any) {
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Value, v)
	if v == nil {
		protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	} else {
		protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 7)
	}
}

func (x *SetRequest) SetNamespace(v string) {
	x.xxx_hidden_Namespace = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 7)
}

func (x *SetRequest) SetTtl(v *durationpb.Duration) {
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Ttl, v)
	if v == nil {
		protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	} else {
		protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 7)
	}
}

func (x *SetRequest) SetMetadata(v *commonpb.RequestMetadata) {
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Metadata, v)
	if v == nil {
		protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	} else {
		protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 7)
	}
}

func (x *SetRequest) SetOverwrite(v bool) {
	x.xxx_hidden_Overwrite = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 7)
}

func (x *SetRequest) SetEntryMetadata(v map[string]string) {
	x.xxx_hidden_EntryMetadata = v
}

func (x *SetRequest) HasKey() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *SetRequest) HasValue() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *SetRequest) HasNamespace() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *SetRequest) HasTtl() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *SetRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *SetRequest) HasOverwrite() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *SetRequest) ClearKey() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Key = nil
}

func (x *SetRequest) ClearValue() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Value, (*anypb.Any)(nil))
}

func (x *SetRequest) ClearNamespace() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_Namespace = nil
}

func (x *SetRequest) ClearTtl() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Ttl, (*durationpb.Duration)(nil))
}

func (x *SetRequest) ClearMetadata() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Metadata, (*commonpb.RequestMetadata)(nil))
}

func (x *SetRequest) ClearOverwrite() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_Overwrite = false
}

type SetRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Cache key to store
	Key *string
	// Value to store (supports any type)
	Value *anypb.Any
	// Optional namespace for cache isolation
	Namespace *string
	// Time-to-live for the cache entry (0 for no expiration)
	Ttl *durationpb.Duration
	// Request metadata for tracing and correlation
	Metadata *commonpb.RequestMetadata
	// Whether to overwrite existing value
	Overwrite *bool
	// Entry metadata for extensibility
	EntryMetadata map[string]string
}

func (b0 SetRequest_builder) Build() *SetRequest {
	m0 := &SetRequest{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Key != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 7)
		x.xxx_hidden_Key = b.Key
	}
	if b.Value != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 7)
		x.xxx_hidden_Value = b.Value
	}
	if b.Namespace != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 7)
		x.xxx_hidden_Namespace = b.Namespace
	}
	if b.Ttl != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 7)
		x.xxx_hidden_Ttl = b.Ttl
	}
	if b.Metadata != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 7)
		x.xxx_hidden_Metadata = b.Metadata
	}
	if b.Overwrite != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 7)
		x.xxx_hidden_Overwrite = *b.Overwrite
	}
	x.xxx_hidden_EntryMetadata = b.EntryMetadata
	return m0
}

var File_databasepb_v1_set_request_proto protoreflect.FileDescriptor

const file_databasepb_v1_set_request_proto_rawDesc = "" +
	"\n" +
	"\x1fdatabasepb/v1/set_request.proto\x12\vdatabase.v1\x1a\"commonpb/v1/request_metadata.proto\x1a\x19google/protobuf/any.proto\x1a\x1egoogle/protobuf/duration.proto\x1a!google/protobuf/go_features.proto\x1a\x1bbuf/validate/validate.proto\"\xa2\x03\n" +
	"\n" +
	"SetRequest\x12\x19\n" +
	"\x03key\x18\x01 \x01(\tB\a\xbaH\x04r\x02\x10\x01R\x03key\x12.\n" +
	"\x05value\x18\x02 \x01(\v2\x14.google.protobuf.AnyB\x02(\x01R\x05value\x12%\n" +
	"\tnamespace\x18\x03 \x01(\tB\a\xbaH\x04r\x02\x10\x01R\tnamespace\x12/\n" +
	"\x03ttl\x18\x04 \x01(\v2\x19.google.protobuf.DurationB\x02(\x01R\x03ttl\x12:\n" +
	"\bmetadata\x18\x05 \x01(\v2\x1a.common.v1.RequestMetadataB\x02(\x01R\bmetadata\x12\x1c\n" +
	"\toverwrite\x18\x06 \x01(\bR\toverwrite\x12U\n" +
	"\x0eentry_metadata\x18\a \x03(\v2*.database.v1.SetRequest.EntryMetadataEntryB\x02(\x01R\rentryMetadata\x1a@\n" +
	"\x12EntryMetadataEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\xa1\x01\n" +
	"\x0fcom.database.v1B\x0fSetRequestProtoP\x01Z(github.com/jdfalk/gcommon/pkg/databasepb\xa2\x02\x03DXX\xaa\x02\vDatabase.V1\xca\x02\vDatabase\\V1\xe2\x02\x17Database\\V1\\GPBMetadata\xea\x02\fDatabase::V1\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_databasepb_v1_set_request_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_databasepb_v1_set_request_proto_goTypes = []any{
	(*SetRequest)(nil),               // 0: database.v1.SetRequest
	nil,                              // 1: database.v1.SetRequest.EntryMetadataEntry
	(*anypb.Any)(nil),                // 2: google.protobuf.Any
	(*durationpb.Duration)(nil),      // 3: google.protobuf.Duration
	(*commonpb.RequestMetadata)(nil), // 4: common.v1.RequestMetadata
}
var file_databasepb_v1_set_request_proto_depIdxs = []int32{
	2, // 0: database.v1.SetRequest.value:type_name -> google.protobuf.Any
	3, // 1: database.v1.SetRequest.ttl:type_name -> google.protobuf.Duration
	4, // 2: database.v1.SetRequest.metadata:type_name -> common.v1.RequestMetadata
	1, // 3: database.v1.SetRequest.entry_metadata:type_name -> database.v1.SetRequest.EntryMetadataEntry
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_databasepb_v1_set_request_proto_init() }
func file_databasepb_v1_set_request_proto_init() {
	if File_databasepb_v1_set_request_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_databasepb_v1_set_request_proto_rawDesc), len(file_databasepb_v1_set_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_databasepb_v1_set_request_proto_goTypes,
		DependencyIndexes: file_databasepb_v1_set_request_proto_depIdxs,
		MessageInfos:      file_databasepb_v1_set_request_proto_msgTypes,
	}.Build()
	File_databasepb_v1_set_request_proto = out.File
	file_databasepb_v1_set_request_proto_goTypes = nil
	file_databasepb_v1_set_request_proto_depIdxs = nil
}
