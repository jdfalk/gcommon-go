// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: databasepb/v1/query_options.proto

package databasepb

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	commonpb "github.com/jdfalk/gcommon/pkg/commonpb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// QueryOptions configures behavior for database query operations.
// Controls result limits, timeouts, and consistency requirements.
type QueryOptions struct {
	state                      protoimpl.MessageState            `protogen:"opaque.v1"`
	xxx_hidden_Limit           int32                             `protobuf:"varint,1,opt,name=limit"`
	xxx_hidden_Offset          int32                             `protobuf:"varint,2,opt,name=offset"`
	xxx_hidden_Timeout         *durationpb.Duration              `protobuf:"bytes,3,opt,name=timeout"`
	xxx_hidden_IncludeMetadata bool                              `protobuf:"varint,4,opt,name=include_metadata,json=includeMetadata"`
	xxx_hidden_Consistency     commonpb.DatabaseConsistencyLevel `protobuf:"varint,5,opt,name=consistency,enum=common.v1.DatabaseConsistencyLevel"`
	// Deprecated: Do not use. This will be deleted in the near future.
	XXX_lazyUnmarshalInfo  protoimpl.LazyUnmarshalInfo
	XXX_raceDetectHookData protoimpl.RaceDetectHookData
	XXX_presence           [1]uint32
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *QueryOptions) Reset() {
	*x = QueryOptions{}
	mi := &file_databasepb_v1_query_options_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QueryOptions) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryOptions) ProtoMessage() {}

func (x *QueryOptions) ProtoReflect() protoreflect.Message {
	mi := &file_databasepb_v1_query_options_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QueryOptions) GetLimit() int32 {
	if x != nil {
		return x.xxx_hidden_Limit
	}
	return 0
}

func (x *QueryOptions) GetOffset() int32 {
	if x != nil {
		return x.xxx_hidden_Offset
	}
	return 0
}

func (x *QueryOptions) GetTimeout() *durationpb.Duration {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 2) {
			if protoimpl.X.AtomicCheckPointerIsNil(&x.xxx_hidden_Timeout) {
				protoimpl.X.UnmarshalField(x, 3)
			}
			var rv *durationpb.Duration
			protoimpl.X.AtomicLoadPointer(protoimpl.Pointer(&x.xxx_hidden_Timeout), protoimpl.Pointer(&rv))
			return rv
		}
	}
	return nil
}

func (x *QueryOptions) GetIncludeMetadata() bool {
	if x != nil {
		return x.xxx_hidden_IncludeMetadata
	}
	return false
}

func (x *QueryOptions) GetConsistency() commonpb.DatabaseConsistencyLevel {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 4) {
			return x.xxx_hidden_Consistency
		}
	}
	return commonpb.DatabaseConsistencyLevel(0)
}

func (x *QueryOptions) SetLimit(v int32) {
	x.xxx_hidden_Limit = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 5)
}

func (x *QueryOptions) SetOffset(v int32) {
	x.xxx_hidden_Offset = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 5)
}

func (x *QueryOptions) SetTimeout(v *durationpb.Duration) {
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Timeout, v)
	if v == nil {
		protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	} else {
		protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 5)
	}
}

func (x *QueryOptions) SetIncludeMetadata(v bool) {
	x.xxx_hidden_IncludeMetadata = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 5)
}

func (x *QueryOptions) SetConsistency(v commonpb.DatabaseConsistencyLevel) {
	x.xxx_hidden_Consistency = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 5)
}

func (x *QueryOptions) HasLimit() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *QueryOptions) HasOffset() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *QueryOptions) HasTimeout() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *QueryOptions) HasIncludeMetadata() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *QueryOptions) HasConsistency() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *QueryOptions) ClearLimit() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Limit = 0
}

func (x *QueryOptions) ClearOffset() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_Offset = 0
}

func (x *QueryOptions) ClearTimeout() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	protoimpl.X.AtomicSetPointer(&x.xxx_hidden_Timeout, (*durationpb.Duration)(nil))
}

func (x *QueryOptions) ClearIncludeMetadata() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_IncludeMetadata = false
}

func (x *QueryOptions) ClearConsistency() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_Consistency = commonpb.DatabaseConsistencyLevel_DATABASE_CONSISTENCY_LEVEL_UNSPECIFIED
}

type QueryOptions_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Maximum number of rows to return
	Limit *int32
	// Number of rows to skip for pagination
	Offset *int32
	// Query execution timeout
	Timeout *durationpb.Duration
	// Whether to include column metadata in response
	IncludeMetadata *bool
	// Read consistency level for the query
	Consistency *commonpb.DatabaseConsistencyLevel
}

func (b0 QueryOptions_builder) Build() *QueryOptions {
	m0 := &QueryOptions{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Limit != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 5)
		x.xxx_hidden_Limit = *b.Limit
	}
	if b.Offset != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 5)
		x.xxx_hidden_Offset = *b.Offset
	}
	if b.Timeout != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 5)
		x.xxx_hidden_Timeout = b.Timeout
	}
	if b.IncludeMetadata != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 5)
		x.xxx_hidden_IncludeMetadata = *b.IncludeMetadata
	}
	if b.Consistency != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 5)
		x.xxx_hidden_Consistency = *b.Consistency
	}
	return m0
}

var File_databasepb_v1_query_options_proto protoreflect.FileDescriptor

const file_databasepb_v1_query_options_proto_rawDesc = "" +
	"\n" +
	"!databasepb/v1/query_options.proto\x12\vdatabase.v1\x1a#commonpb/v1/consistency_level.proto\x1a\x1egoogle/protobuf/duration.proto\x1a!google/protobuf/go_features.proto\x1a\x1bbuf/validate/validate.proto\"\xf9\x01\n" +
	"\fQueryOptions\x12\x1d\n" +
	"\x05limit\x18\x01 \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x05limit\x12\x1f\n" +
	"\x06offset\x18\x02 \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x06offset\x127\n" +
	"\atimeout\x18\x03 \x01(\v2\x19.google.protobuf.DurationB\x02(\x01R\atimeout\x12)\n" +
	"\x10include_metadata\x18\x04 \x01(\bR\x0fincludeMetadata\x12E\n" +
	"\vconsistency\x18\x05 \x01(\x0e2#.common.v1.DatabaseConsistencyLevelR\vconsistencyB\xa3\x01\n" +
	"\x0fcom.database.v1B\x11QueryOptionsProtoP\x01Z(github.com/jdfalk/gcommon/pkg/databasepb\xa2\x02\x03DXX\xaa\x02\vDatabase.V1\xca\x02\vDatabase\\V1\xe2\x02\x17Database\\V1\\GPBMetadata\xea\x02\fDatabase::V1\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_databasepb_v1_query_options_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_databasepb_v1_query_options_proto_goTypes = []any{
	(*QueryOptions)(nil),                   // 0: database.v1.QueryOptions
	(*durationpb.Duration)(nil),            // 1: google.protobuf.Duration
	(commonpb.DatabaseConsistencyLevel)(0), // 2: common.v1.DatabaseConsistencyLevel
}
var file_databasepb_v1_query_options_proto_depIdxs = []int32{
	1, // 0: database.v1.QueryOptions.timeout:type_name -> google.protobuf.Duration
	2, // 1: database.v1.QueryOptions.consistency:type_name -> common.v1.DatabaseConsistencyLevel
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_databasepb_v1_query_options_proto_init() }
func file_databasepb_v1_query_options_proto_init() {
	if File_databasepb_v1_query_options_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_databasepb_v1_query_options_proto_rawDesc), len(file_databasepb_v1_query_options_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_databasepb_v1_query_options_proto_goTypes,
		DependencyIndexes: file_databasepb_v1_query_options_proto_depIdxs,
		MessageInfos:      file_databasepb_v1_query_options_proto_msgTypes,
	}.Build()
	File_databasepb_v1_query_options_proto = out.File
	file_databasepb_v1_query_options_proto_goTypes = nil
	file_databasepb_v1_query_options_proto_depIdxs = nil
}
