// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: metrics/v2/record_histogram_request.proto

package v2

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	v2 "github.com/jdfalk/gcommon/pkg/common/v2"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RecordHistogramRequest struct {
	state                      protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Name            *string                `protobuf:"bytes,1,opt,name=name"`
	xxx_hidden_Value           float64                `protobuf:"fixed64,2,opt,name=value"`
	xxx_hidden_Labels          map[string]string      `protobuf:"bytes,3,rep,name=labels" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_Help            *string                `protobuf:"bytes,4,opt,name=help"`
	xxx_hidden_Unit            *string                `protobuf:"bytes,5,opt,name=unit"`
	xxx_hidden_Buckets         []float64              `protobuf:"fixed64,6,rep,packed,name=buckets"`
	xxx_hidden_Timestamp       *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=timestamp"`
	xxx_hidden_Metadata        *v2.RequestMetadata    `protobuf:"bytes,8,opt,name=metadata"`
	xxx_hidden_SampleWeight    float64                `protobuf:"fixed64,9,opt,name=sample_weight,json=sampleWeight"`
	xxx_hidden_CreateIfMissing bool                   `protobuf:"varint,10,opt,name=create_if_missing,json=createIfMissing"`
	XXX_raceDetectHookData     protoimpl.RaceDetectHookData
	XXX_presence               [1]uint32
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *RecordHistogramRequest) Reset() {
	*x = RecordHistogramRequest{}
	mi := &file_metrics_v2_record_histogram_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RecordHistogramRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RecordHistogramRequest) ProtoMessage() {}

func (x *RecordHistogramRequest) ProtoReflect() protoreflect.Message {
	mi := &file_metrics_v2_record_histogram_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *RecordHistogramRequest) GetName() string {
	if x != nil {
		if x.xxx_hidden_Name != nil {
			return *x.xxx_hidden_Name
		}
		return ""
	}
	return ""
}

func (x *RecordHistogramRequest) GetValue() float64 {
	if x != nil {
		return x.xxx_hidden_Value
	}
	return 0
}

func (x *RecordHistogramRequest) GetLabels() map[string]string {
	if x != nil {
		return x.xxx_hidden_Labels
	}
	return nil
}

func (x *RecordHistogramRequest) GetHelp() string {
	if x != nil {
		if x.xxx_hidden_Help != nil {
			return *x.xxx_hidden_Help
		}
		return ""
	}
	return ""
}

func (x *RecordHistogramRequest) GetUnit() string {
	if x != nil {
		if x.xxx_hidden_Unit != nil {
			return *x.xxx_hidden_Unit
		}
		return ""
	}
	return ""
}

func (x *RecordHistogramRequest) GetBuckets() []float64 {
	if x != nil {
		return x.xxx_hidden_Buckets
	}
	return nil
}

func (x *RecordHistogramRequest) GetTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.xxx_hidden_Timestamp
	}
	return nil
}

func (x *RecordHistogramRequest) GetMetadata() *v2.RequestMetadata {
	if x != nil {
		return x.xxx_hidden_Metadata
	}
	return nil
}

func (x *RecordHistogramRequest) GetSampleWeight() float64 {
	if x != nil {
		return x.xxx_hidden_SampleWeight
	}
	return 0
}

func (x *RecordHistogramRequest) GetCreateIfMissing() bool {
	if x != nil {
		return x.xxx_hidden_CreateIfMissing
	}
	return false
}

func (x *RecordHistogramRequest) SetName(v string) {
	x.xxx_hidden_Name = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 10)
}

func (x *RecordHistogramRequest) SetValue(v float64) {
	x.xxx_hidden_Value = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 10)
}

func (x *RecordHistogramRequest) SetLabels(v map[string]string) {
	x.xxx_hidden_Labels = v
}

func (x *RecordHistogramRequest) SetHelp(v string) {
	x.xxx_hidden_Help = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 10)
}

func (x *RecordHistogramRequest) SetUnit(v string) {
	x.xxx_hidden_Unit = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 10)
}

func (x *RecordHistogramRequest) SetBuckets(v []float64) {
	x.xxx_hidden_Buckets = v
}

func (x *RecordHistogramRequest) SetTimestamp(v *timestamppb.Timestamp) {
	x.xxx_hidden_Timestamp = v
}

func (x *RecordHistogramRequest) SetMetadata(v *v2.RequestMetadata) {
	x.xxx_hidden_Metadata = v
}

func (x *RecordHistogramRequest) SetSampleWeight(v float64) {
	x.xxx_hidden_SampleWeight = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 10)
}

func (x *RecordHistogramRequest) SetCreateIfMissing(v bool) {
	x.xxx_hidden_CreateIfMissing = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 9, 10)
}

func (x *RecordHistogramRequest) HasName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *RecordHistogramRequest) HasValue() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *RecordHistogramRequest) HasHelp() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *RecordHistogramRequest) HasUnit() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *RecordHistogramRequest) HasTimestamp() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Timestamp != nil
}

func (x *RecordHistogramRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Metadata != nil
}

func (x *RecordHistogramRequest) HasSampleWeight() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *RecordHistogramRequest) HasCreateIfMissing() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 9)
}

func (x *RecordHistogramRequest) ClearName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Name = nil
}

func (x *RecordHistogramRequest) ClearValue() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_Value = 0
}

func (x *RecordHistogramRequest) ClearHelp() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_Help = nil
}

func (x *RecordHistogramRequest) ClearUnit() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_Unit = nil
}

func (x *RecordHistogramRequest) ClearTimestamp() {
	x.xxx_hidden_Timestamp = nil
}

func (x *RecordHistogramRequest) ClearMetadata() {
	x.xxx_hidden_Metadata = nil
}

func (x *RecordHistogramRequest) ClearSampleWeight() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_SampleWeight = 0
}

func (x *RecordHistogramRequest) ClearCreateIfMissing() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 9)
	x.xxx_hidden_CreateIfMissing = false
}

type RecordHistogramRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Metric name (e.g., "request_duration_seconds", "response_size_bytes")
	Name *string
	// Value to observe/record in the histogram
	Value *float64
	// Labels for metric dimensions
	Labels map[string]string
	// Help text describing the metric
	Help *string
	// Metric unit (e.g., "seconds", "bytes", "milliseconds")
	Unit *string
	// Histogram bucket configuration (if not already defined)
	Buckets []float64
	// Optional timestamp when the observation was made
	Timestamp *timestamppb.Timestamp
	// Request metadata for tracing and debugging
	Metadata *v2.RequestMetadata
	// Optional sample weight (for weighted observations)
	SampleWeight *float64
	// Whether to create the histogram if it doesn't exist
	CreateIfMissing *bool
}

func (b0 RecordHistogramRequest_builder) Build() *RecordHistogramRequest {
	m0 := &RecordHistogramRequest{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Name != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 10)
		x.xxx_hidden_Name = b.Name
	}
	if b.Value != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 10)
		x.xxx_hidden_Value = *b.Value
	}
	x.xxx_hidden_Labels = b.Labels
	if b.Help != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 10)
		x.xxx_hidden_Help = b.Help
	}
	if b.Unit != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 10)
		x.xxx_hidden_Unit = b.Unit
	}
	x.xxx_hidden_Buckets = b.Buckets
	x.xxx_hidden_Timestamp = b.Timestamp
	x.xxx_hidden_Metadata = b.Metadata
	if b.SampleWeight != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 10)
		x.xxx_hidden_SampleWeight = *b.SampleWeight
	}
	if b.CreateIfMissing != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 9, 10)
		x.xxx_hidden_CreateIfMissing = *b.CreateIfMissing
	}
	return m0
}

var File_metrics_v2_record_histogram_request_proto protoreflect.FileDescriptor

const file_metrics_v2_record_histogram_request_proto_rawDesc = "" +
	"\n" +
	")metrics/v2/record_histogram_request.proto\x12\n" +
	"metrics.v2\x1a common/v2/request_metadata.proto\x1a!google/protobuf/go_features.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1bbuf/validate/validate.proto\"\xd5\x03\n" +
	"\x16RecordHistogramRequest\x12\x1d\n" +
	"\x04name\x18\x01 \x01(\tB\t\xbaH\x06r\x04\x10\x01\x18dR\x04name\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value\x12F\n" +
	"\x06labels\x18\x03 \x03(\v2..metrics.v2.RecordHistogramRequest.LabelsEntryR\x06labels\x12\x12\n" +
	"\x04help\x18\x04 \x01(\tR\x04help\x12\x12\n" +
	"\x04unit\x18\x05 \x01(\tR\x04unit\x12\x18\n" +
	"\abuckets\x18\x06 \x03(\x01R\abuckets\x128\n" +
	"\ttimestamp\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\ttimestamp\x126\n" +
	"\bmetadata\x18\b \x01(\v2\x1a.common.v2.RequestMetadataR\bmetadata\x12#\n" +
	"\rsample_weight\x18\t \x01(\x01R\fsampleWeight\x12*\n" +
	"\x11create_if_missing\x18\n" +
	" \x01(\bR\x0fcreateIfMissing\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B2Z(github.com/jdfalk/gcommon/pkg/metrics/v2\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_metrics_v2_record_histogram_request_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_metrics_v2_record_histogram_request_proto_goTypes = []any{
	(*RecordHistogramRequest)(nil), // 0: metrics.v2.RecordHistogramRequest
	nil,                            // 1: metrics.v2.RecordHistogramRequest.LabelsEntry
	(*timestamppb.Timestamp)(nil),  // 2: google.protobuf.Timestamp
	(*v2.RequestMetadata)(nil),     // 3: common.v2.RequestMetadata
}
var file_metrics_v2_record_histogram_request_proto_depIdxs = []int32{
	1, // 0: metrics.v2.RecordHistogramRequest.labels:type_name -> metrics.v2.RecordHistogramRequest.LabelsEntry
	2, // 1: metrics.v2.RecordHistogramRequest.timestamp:type_name -> google.protobuf.Timestamp
	3, // 2: metrics.v2.RecordHistogramRequest.metadata:type_name -> common.v2.RequestMetadata
	3, // [3:3] is the sub-list for method output_type
	3, // [3:3] is the sub-list for method input_type
	3, // [3:3] is the sub-list for extension type_name
	3, // [3:3] is the sub-list for extension extendee
	0, // [0:3] is the sub-list for field type_name
}

func init() { file_metrics_v2_record_histogram_request_proto_init() }
func file_metrics_v2_record_histogram_request_proto_init() {
	if File_metrics_v2_record_histogram_request_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_metrics_v2_record_histogram_request_proto_rawDesc), len(file_metrics_v2_record_histogram_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_metrics_v2_record_histogram_request_proto_goTypes,
		DependencyIndexes: file_metrics_v2_record_histogram_request_proto_depIdxs,
		MessageInfos:      file_metrics_v2_record_histogram_request_proto_msgTypes,
	}.Build()
	File_metrics_v2_record_histogram_request_proto = out.File
	file_metrics_v2_record_histogram_request_proto_goTypes = nil
	file_metrics_v2_record_histogram_request_proto_depIdxs = nil
}
