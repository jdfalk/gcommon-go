// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        (unknown)
// source: metrics/v1/unregistration_options.proto

package v1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type UnregistrationOptions struct {
	state                    protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_DeleteData    bool                   `protobuf:"varint,1,opt,name=delete_data,json=deleteData"`
	xxx_hidden_DeleteIndices bool                   `protobuf:"varint,2,opt,name=delete_indices,json=deleteIndices"`
	xxx_hidden_RemoveAlerts  bool                   `protobuf:"varint,3,opt,name=remove_alerts,json=removeAlerts"`
	xxx_hidden_StopExports   bool                   `protobuf:"varint,4,opt,name=stop_exports,json=stopExports"`
	xxx_hidden_GracePeriod   *string                `protobuf:"bytes,5,opt,name=grace_period,json=gracePeriod"`
	xxx_hidden_DryRun        bool                   `protobuf:"varint,6,opt,name=dry_run,json=dryRun"`
	xxx_hidden_Force         bool                   `protobuf:"varint,7,opt,name=force"`
	xxx_hidden_CreateBackup  bool                   `protobuf:"varint,8,opt,name=create_backup,json=createBackup"`
	XXX_raceDetectHookData   protoimpl.RaceDetectHookData
	XXX_presence             [1]uint32
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *UnregistrationOptions) Reset() {
	*x = UnregistrationOptions{}
	mi := &file_metrics_v1_unregistration_options_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnregistrationOptions) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnregistrationOptions) ProtoMessage() {}

func (x *UnregistrationOptions) ProtoReflect() protoreflect.Message {
	mi := &file_metrics_v1_unregistration_options_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *UnregistrationOptions) GetDeleteData() bool {
	if x != nil {
		return x.xxx_hidden_DeleteData
	}
	return false
}

func (x *UnregistrationOptions) GetDeleteIndices() bool {
	if x != nil {
		return x.xxx_hidden_DeleteIndices
	}
	return false
}

func (x *UnregistrationOptions) GetRemoveAlerts() bool {
	if x != nil {
		return x.xxx_hidden_RemoveAlerts
	}
	return false
}

func (x *UnregistrationOptions) GetStopExports() bool {
	if x != nil {
		return x.xxx_hidden_StopExports
	}
	return false
}

func (x *UnregistrationOptions) GetGracePeriod() string {
	if x != nil {
		if x.xxx_hidden_GracePeriod != nil {
			return *x.xxx_hidden_GracePeriod
		}
		return ""
	}
	return ""
}

func (x *UnregistrationOptions) GetDryRun() bool {
	if x != nil {
		return x.xxx_hidden_DryRun
	}
	return false
}

func (x *UnregistrationOptions) GetForce() bool {
	if x != nil {
		return x.xxx_hidden_Force
	}
	return false
}

func (x *UnregistrationOptions) GetCreateBackup() bool {
	if x != nil {
		return x.xxx_hidden_CreateBackup
	}
	return false
}

func (x *UnregistrationOptions) SetDeleteData(v bool) {
	x.xxx_hidden_DeleteData = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 8)
}

func (x *UnregistrationOptions) SetDeleteIndices(v bool) {
	x.xxx_hidden_DeleteIndices = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 8)
}

func (x *UnregistrationOptions) SetRemoveAlerts(v bool) {
	x.xxx_hidden_RemoveAlerts = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 8)
}

func (x *UnregistrationOptions) SetStopExports(v bool) {
	x.xxx_hidden_StopExports = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 8)
}

func (x *UnregistrationOptions) SetGracePeriod(v string) {
	x.xxx_hidden_GracePeriod = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 8)
}

func (x *UnregistrationOptions) SetDryRun(v bool) {
	x.xxx_hidden_DryRun = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 8)
}

func (x *UnregistrationOptions) SetForce(v bool) {
	x.xxx_hidden_Force = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 8)
}

func (x *UnregistrationOptions) SetCreateBackup(v bool) {
	x.xxx_hidden_CreateBackup = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 8)
}

func (x *UnregistrationOptions) HasDeleteData() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *UnregistrationOptions) HasDeleteIndices() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *UnregistrationOptions) HasRemoveAlerts() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *UnregistrationOptions) HasStopExports() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *UnregistrationOptions) HasGracePeriod() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *UnregistrationOptions) HasDryRun() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *UnregistrationOptions) HasForce() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *UnregistrationOptions) HasCreateBackup() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *UnregistrationOptions) ClearDeleteData() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_DeleteData = false
}

func (x *UnregistrationOptions) ClearDeleteIndices() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_DeleteIndices = false
}

func (x *UnregistrationOptions) ClearRemoveAlerts() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_RemoveAlerts = false
}

func (x *UnregistrationOptions) ClearStopExports() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_StopExports = false
}

func (x *UnregistrationOptions) ClearGracePeriod() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_GracePeriod = nil
}

func (x *UnregistrationOptions) ClearDryRun() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_DryRun = false
}

func (x *UnregistrationOptions) ClearForce() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_Force = false
}

func (x *UnregistrationOptions) ClearCreateBackup() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_CreateBackup = false
}

type UnregistrationOptions_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Whether to delete all associated data
	DeleteData *bool
	// Whether to delete associated indices
	DeleteIndices *bool
	// Whether to remove alert rules
	RemoveAlerts *bool
	// Whether to stop export configurations
	StopExports *bool
	// Grace period before actual deletion (duration string like "24h")
	GracePeriod *string
	// Whether to perform a dry run (show what would be deleted)
	DryRun *bool
	// Whether to force deletion even if metric is in use
	Force *bool
	// Whether to create a backup before deletion
	CreateBackup *bool
}

func (b0 UnregistrationOptions_builder) Build() *UnregistrationOptions {
	m0 := &UnregistrationOptions{}
	b, x := &b0, m0
	_, _ = b, x
	if b.DeleteData != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 8)
		x.xxx_hidden_DeleteData = *b.DeleteData
	}
	if b.DeleteIndices != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 8)
		x.xxx_hidden_DeleteIndices = *b.DeleteIndices
	}
	if b.RemoveAlerts != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 8)
		x.xxx_hidden_RemoveAlerts = *b.RemoveAlerts
	}
	if b.StopExports != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 8)
		x.xxx_hidden_StopExports = *b.StopExports
	}
	if b.GracePeriod != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 8)
		x.xxx_hidden_GracePeriod = b.GracePeriod
	}
	if b.DryRun != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 8)
		x.xxx_hidden_DryRun = *b.DryRun
	}
	if b.Force != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 8)
		x.xxx_hidden_Force = *b.Force
	}
	if b.CreateBackup != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 8)
		x.xxx_hidden_CreateBackup = *b.CreateBackup
	}
	return m0
}

var File_metrics_v1_unregistration_options_proto protoreflect.FileDescriptor

const file_metrics_v1_unregistration_options_proto_rawDesc = "" +
	"\n" +
	"'metrics/v1/unregistration_options.proto\x12\n" +
	"metrics.v1\x1a!google/protobuf/go_features.proto\x1a\x1bbuf/validate/validate.proto\"\xa7\x02\n" +
	"\x15UnregistrationOptions\x12\x1f\n" +
	"\vdelete_data\x18\x01 \x01(\bR\n" +
	"deleteData\x12%\n" +
	"\x0edelete_indices\x18\x02 \x01(\bR\rdeleteIndices\x12#\n" +
	"\rremove_alerts\x18\x03 \x01(\bR\fremoveAlerts\x12!\n" +
	"\fstop_exports\x18\x04 \x01(\bR\vstopExports\x12*\n" +
	"\fgrace_period\x18\x05 \x01(\tB\a\xbaH\x04r\x02\x10\x01R\vgracePeriod\x12\x17\n" +
	"\adry_run\x18\x06 \x01(\bR\x06dryRun\x12\x14\n" +
	"\x05force\x18\a \x01(\bR\x05force\x12#\n" +
	"\rcreate_backup\x18\b \x01(\bR\fcreateBackupB.Z$github.com/jdfalk/gcommon/metrics/v1\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_metrics_v1_unregistration_options_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_metrics_v1_unregistration_options_proto_goTypes = []any{
	(*UnregistrationOptions)(nil), // 0: metrics.v1.UnregistrationOptions
}
var file_metrics_v1_unregistration_options_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_metrics_v1_unregistration_options_proto_init() }
func file_metrics_v1_unregistration_options_proto_init() {
	if File_metrics_v1_unregistration_options_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_metrics_v1_unregistration_options_proto_rawDesc), len(file_metrics_v1_unregistration_options_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_metrics_v1_unregistration_options_proto_goTypes,
		DependencyIndexes: file_metrics_v1_unregistration_options_proto_depIdxs,
		MessageInfos:      file_metrics_v1_unregistration_options_proto_msgTypes,
	}.Build()
	File_metrics_v1_unregistration_options_proto = out.File
	file_metrics_v1_unregistration_options_proto_goTypes = nil
	file_metrics_v1_unregistration_options_proto_depIdxs = nil
}
